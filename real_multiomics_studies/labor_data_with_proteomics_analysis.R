...
## this script performs the analysis of labor data using features from the proteome as modifying variables
...
met_G1 = met[G1_index,]
immun_G1 = immun[G1_index,]
x1_fil = as.matrix(prot_G1)
x2_fil = as.matrix(met_G1)
z_fil = as.matrix(prot_G1)
y = DOS[G1_index]
# x1_fil = prot
# x2_fil = met
# z_fil = immun
# z_fil <- z_fil[ , colSums(is.na(z_fil))==0]
# y = DOS
# X1 = Proteomics
# X2 = Metabolomics
# Z = immun
...
for (j in 1:simN){
  ...
  # Z_ind = which(log(apply(train_Z_raw,2,var)) > 3.5)
  ...
  varZ <- log(apply(train_Z_raw,2,var))
  varZ_sort <- sort(varZ, decreasing=TRUE)
  sum.a <- cumsum(varZ_sort)
  
  #half.a <- var.sort[which(sum.a>(sum.a[length(varZ)]*0.1))[1]] # explaining 10% variations
  train_Z_raw <- as.matrix(train_Z_raw[,varZ>=varZ_sort[5]])
  test_Z_raw <- as.matrix(test_Z_raw[,varZ>=varZ_sort[5]])
  ...
  only_X1_fit <- pliable(x=X1_train,z=Z_train,y = y_train, nlambda=50, zlinear=FALSE, alpha = 0.1, tt=0.01) #, zlinear = FALSE
  ...
  only_X2_fit <- pliable(x=X2_train,z=Z_train,y = y_train, nlambda=50, zlinear=FALSE, alpha = 0.1, tt=0.01)
  ...
  early_fit <- pliable(x=X_train,z=Z_train,y = y_train, nlambda=50, zlinear=FALSE, alpha = 0.1, tt=0.01)
  ...
  X1_plasso_fit_late = pliable(X1_train_late, Z_train_late, y_train_late, zlinear=FALSE, alpha = 0.1, tt=0.01)
  cv_X1_plasso_fit_late = cv.pliable(X1_plasso_fit_late, X1_train_late, Z_train_late, y_train_late, nfolds=5, verbose=FALSE)
  
  X2_plasso_fit_late = pliable(X2_train_late, Z_train_late, y_train_late, zlinear=FALSE, alpha = 0.1, tt=0.01)
  ...
  for (i in seq(length(rhos))){
    ...
    full_fit_no_pf = coop_cv_new_pliable(X1_train,X2_train,Z_train,y_train,
                                         ...
                                         nfolds=max(foldid), alpha_pliable = 0.1, tt=0.05)
  ...
  #                                             alpha=rho,foldid=foldid,
  #                                             nfolds=max(foldid),pf_values=c(rep(1,ncol(X1_train)),rep(lambda2/lambda1,ncol(X2_train))))
  # 
  # 
  ...
  #   test_mse_no_pf_adap[i] = calc_mse(y_test,predict(full_fit_no_pf_adap$best_fit,cbind(X1_test,X2_test),Z_test)[,full_fit_no_pf_adap$min_ind])
...
saveRDS(labor_onset_mat, paste0("C:/Users/dalma/Desktop/Matteo/uni/magistrale/erasmus/thesis/labor_onset_with_immunomics_results.rds"))
write(c(mean(chosen_rhos),mean(chosen_rhos_adap)), file = "C:/Users/dalma/Desktop/Matteo/uni/magistrale/erasmus/thesis/labor_onset_with_immunomics_chosen_rhos.txt")
...
for(j in 1:simN){
  avg_abs_beta_x1[[j]] = abs(beta_matrixes[[1 + (j-1)*5]])
}
...
for(j in 1:simN){
  avg_abs_theta_x1[[j]] = theta_matrixes[[1 + (j-1)*5]]!=0
}
...
A1 <- matrix(0, ncol=5, nrow=length(names_x1), dimnames=list(names_x1, NULL))
...
for(j in 1:simN){
  avg_abs_theta_x2[[j]] = theta_matrixes[[2 + (j-1)*5]]!=0
}
...
A2 <- matrix(0, ncol=5, nrow=length(names_x2), dimnames=list(names_x2, NULL))
...
for(j in 1:simN){
  avg_abs_theta_early[[j]] = theta_matrixes[[3 + (j-1)*5]]!=0
  names(avg_abs_theta_early[[j]]) = names(theta_matrixes[[3 + (j-1)*5]])
}
...
Aearly <- matrix(0, ncol=5, nrow=length(names_early), dimnames=list(names_early, NULL))
...
for(j in 1:simN){
  avg_abs_theta_late[[j]] = theta_matrixes[[4 + (j-1)*5]]!=0
  names(avg_abs_theta_late[[j]]) = names(theta_matrixes[[4 + (j-1)*5]])
}
...
Alate <- matrix(0, ncol=5, nrow=length(names_late), dimnames=list(names_late, NULL))
...
for(j in 1:simN){
  avg_abs_theta_coop[[j]] = theta_matrixes[[5 + (j-1)*5]]!=0
  names(avg_abs_theta_coop[[j]]) = names(theta_matrixes[[5 + (j-1)*5]])
}
...
Acoop <- matrix(0, ncol=5, nrow=length(names_coop), dimnames=list(names_coop, NULL))
